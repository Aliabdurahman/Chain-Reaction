...
def resetBoard():
    turn.set(0)
    turnNote.set("It's player " + str(turn.get() + 1)+ " ("+color[turn.get()]+") "  + "'s turn!")
...
def passTurn():
    ...
    turnNote.set("It's player " + str(nextTurn)+" ("+color[nextTurn-1]+") "   + "'s turn!")

#set notification message to win status
def declareWinner():
    turnNote.set("Player " + str((turn.get() % playersCount) + 1)+ " ("+color[(turn.get() % playersCount)]+ ") "  + " WON!")            
